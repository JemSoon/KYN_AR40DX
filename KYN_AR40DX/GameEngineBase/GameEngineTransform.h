...
/////////////////////////// 충돌관련
public:
	static bool SphereToSphere(const GameEngineTransform& _Left, const GameEngineTransform& _Right);//원대원

	static bool AABBToAABB(const GameEngineTransform& _Left, const GameEngineTransform& _Right);//xyz축에 딱 맞춰진 충돌체 충돌

	static bool OBBToOBB(const GameEngineTransform& _Left, const GameEngineTransform& _Right);//xyz축에 지맘대로 회전한 충돌체 충돌

	//z값 필요없는 2D용(위랑 거의 같은데 z값만 0으로 통일시켜 계산한다)
